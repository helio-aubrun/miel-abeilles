...
question_matplotlib = (
    "vouler vous voire le chemin de la meilleir abeille avec matplotlib"
)
...
def not_convergence(values):
    if len(values) >= AVERAGE_COMPARAISON * 2:
        try:
            values_last_gens = list(
                values[-AVERAGE_COMPARAISON * 2 :][:AVERAGE_COMPARAISON]
            )
            values_actual_gens = list(values[-AVERAGE_COMPARAISON:])
            return not (
                0
                <= compute_average_values(values_last_gens)
                - compute_average_values(values_actual_gens)
                <= 50
            )
        except:
            return True
